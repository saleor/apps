import { Box, Modal, type PropsWithBox } from "@saleor/macaw-ui";
...
import { isRadixPortaledContent } from "./is-radix-portaled-content";
import {
  MODAL_ACTIONS_DISPLAY_NAME,
  MODAL_BODY_DISPLAY_NAME,
  MODAL_CHROME_DISPLAY_NAMES,
} from "./modal-display-names";
import { ModalLayoutContextProvider } from "./modal-layout-context";
...
const isModalComponent = (child: ReactNode, displayName: string): boolean =>
  isValidElement(child) && getComponentDisplayName(child.type) === displayName;

const isChromeComponent = (child: ReactNode): boolean =>
  MODAL_CHROME_DISPLAY_NAMES.some((displayName) => isModalComponent(child, displayName));
