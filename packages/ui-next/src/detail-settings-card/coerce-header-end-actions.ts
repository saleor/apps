...
const isHeaderAction = (element: ReactElement): boolean => element.type === Button;

/**
 * Force macaw `Button` in `headerEnd` to small unless the caller already set `size`.
 * Recurses through layout wrappers so nested actions pick it up.
 */
