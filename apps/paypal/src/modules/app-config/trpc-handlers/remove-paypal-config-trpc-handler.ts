  ...
  private async getRootConfigOrThrow({
    ...
    if (rootConfig.isErr()) {
      throw new TRPCError({
        ...
        message: "Failed to remove PayPal configuration. Please try again.",
      });
    }
    ...
    return this.baseProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        if (webhookRemovingResult.isErr()) {
          /**
           * Ignore - it's possible webhook was removed in PayPal, or in previous call. We can't transactional remove both webhook and our config,
           * so we must accept that inconsistency and document it
           */
        }

        /**
         * TODO: To make it more reliable, we can use transact writes in PostgreSQL, but even if this partially fails,
