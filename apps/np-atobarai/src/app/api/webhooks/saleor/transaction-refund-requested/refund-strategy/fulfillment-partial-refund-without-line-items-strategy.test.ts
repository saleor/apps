...
import { FulfillmentPartialRefundWithoutLineItemsStrategy } from "./fulfillment-partial-refund-without-line-items-strategy";
import { FulfillmentRefundContext } from "./types";

describe("FulfillmentPartialRefundWithoutLineItemsStrategy", () => {
  let strategy: FulfillmentPartialRefundWithoutLineItemsStrategy;
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    strategy = new FulfillmentPartialRefundWithoutLineItemsStrategy();
  });
  ...
  it("should execute partial refund without line items successfully", async () => {
    ...
    const expectedAmount =
      baseContext.parsedEvent.sourceObjectTotalAmount - baseContext.parsedEvent.refundedAmount;
