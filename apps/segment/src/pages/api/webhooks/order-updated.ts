...
const handler: NextWebhookApiHandler<OrderUpdatedSubscriptionPayloadFragment> = async (
  ...
  try {
    ...
    await segmentEventTracker.trackEvent(
      trackingEventFactory.createOrderUpdatedEvent({
    ...
    return res.status(200).json({ message: "Order updated event successfully sent to Segment" });
  } catch (e) {
    ...
    logger.error("Error while sending order updated event to Segment", { error: e });
