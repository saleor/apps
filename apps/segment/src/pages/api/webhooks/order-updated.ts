...
import { type OrderUpdatedSubscriptionPayloadFragment } from "@/generated/graphql";
        ...
        (error) => {
          switch (error.constructor) {
            ...
            case TrackEventUseCase.TrackEventUseCaseUnknownError: {
              ...
              return res.status(500).send("Error while sending order updated event to Segment");
            }
          }
        },
      );
...
export default wrapWithLoggerContext(
  withSpanAttributes(orderUpdatedAsyncWebhook.createHandler(handler)),
  loggerContext,
);
