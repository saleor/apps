...
export const setupIndicesHandlerFactory =
  ...
  async (req, res, { authData }) => {
    ...
    const typesenseClient = new TypesenseSearchProvider({
      host: configData.appConfig.host,
      port: configData.appConfig.port,
      protocol: configData.appConfig.protocol,
      apiKey: configData.appConfig.apiKey,
      connectionTimeoutSeconds: configData.appConfig.connectionTimeoutSeconds,
      enabledKeys: configData.fieldsMapping.enabledTypesenseFields,
    });

    try {
      logger.debug("Running indices update");
      await typesenseClient.updateIndicesSettings();
