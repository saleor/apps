...
import { type FulfillmentCanceledWebhookPayloadFragment } from "../../../../generated/graphql";
...
const FulfillmentCanceledWebhookPayload = gql`
...
const FulfillmentCanceledGraphqlSubscription = gql`
...
export const fulfillmentCanceledWebhook =
  new SaleorAsyncWebhook<FulfillmentCanceledWebhookPayloadFragment>({
    name: "Fulfillment Canceled in Saleor",
    webhookPath: "api/webhooks/fulfillment-canceled",
    event: "FULFILLMENT_CANCELED",
    apl: saleorApp.apl,
    query: FulfillmentCanceledGraphqlSubscription,
  });

const logger = createLogger(fulfillmentCanceledWebhook.webhookPath);

const useCaseFactory = new SendEventMessagesUseCaseFactory();

export const handler: NextJsWebhookHandler<FulfillmentCanceledWebhookPayloadFragment> = async (
  req,
  res,
  context,
) => {
...
export default wrapWithLoggerContext(
  withSpanAttributes(fulfillmentCanceledWebhook.createHandler(handler)),
  loggerContext,
);
