...
export const SmtpSection = ({ configuration }: SmtpSectionProps) => {
  ...
  const { mutate, isLoading: isSaving } = trpcClient.smtpConfiguration.updateSmtp.useMutation({
    onSuccess: async () => {
          ...
          <Box padding={defaultPadding} display="flex" flexDirection="column" gap={7}>
            ...
            <Box display="flex" gap={defaultPadding}>
              ...
              <label>
                ...
                <Text paddingLeft={defaultPadding}>Implicit TLS (TLSv1.3+)</Text>
              </label>
              <label>
                <input {...register("encryption")} type="radio" value="TLS" />
                <Text paddingLeft={defaultPadding}>STARTTLS (TLSv1.3+)</Text>
              </label>
            </Box>
          </Box>
          <BoxFooter>
